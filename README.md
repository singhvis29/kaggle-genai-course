...
6. The core component of LLM systems are Prompted Model Components. The Gen AI experimentation cycle typically begins with testing variations of the prompt – changing the wording of the instructions, providing additional context, or including relevant examples, etc., and evaluating the impact of those changes. This practice is commonly referred to as prompt engineering.
7. When applying MLOps practices to gen AI, it is important to have in place processes that give developers easy storage, retrieval, tracking, and modification of prompts. This allows for fast iteration and principled experimentation.
8. Sometimes using one iteration of prompted model is not sufficient to obtain desired results. To solve this issue we leverage a divide and conquer approach, several prompted models can be connected together, along with calls to external APIs and logic expressed as code. A sequence of prompted model components connected together in this way is commonly known as a chain.
9. Two common chain-based patterns that have emerged to mitigate recency and hallucinations are retrieval augmented generation (RAG)3 and Agents.
10. Unlike Predictive AI where it is often possible to iterate on the separate models and components in isolation to then chain in the AI application, in gen AI it’s often easier to develop a chain in integration, performing experimentation on the chain end-to-end, iterating over chaining strategies, prompts, the underlying foundational models and other APIs in
a coordinated manner to achieve a specific goal. No feature engineering, data collection, or further model training cycles is often needed; just changes to the wording of the prompt template.
11. When developing Gen-AI use case, it is sometimes required to fine-tune the model directly, fine-tuning lets us change one or more layers of LLM to output a more desired output. Two method to fine-tune an LLM is Supervised Fine-Tuning, Reinforcement Learning from Human Feedback (RLHF).
12. Services on GCP which are used to set up MLOps pipeline -
   * Vertex Model Registry - provides centralized storage location for all artifacts created during tuning job
   * Vertex Pipelines - streamlines the development and management of these pipelines
   * Dataplex - provides an organization-wide data fabric for data lineage and governance and integrates well with both Vertex AI and BigQuery
13. Continuous training in MLOps involves repeatedly retraining production models as data patterns change, but for Gen AI, continuous tuning is often more practical due to high data and computational costs. Techniques like model quantization can help manage costs by reducing computational and memory requirements through representing model weights and activations using lower-precision integers instead of higher-precision floats.
14. In the context of gen AI applications built on foundation models, data practices shift from focusing solely on model training data to emphasizing the various types of adaptation data used to determine model behavior. This introduces a new complexity in managing diverse data types throughout the development lifecycle. Managing the diverse range of data types used in gen AI adds a layer of complexity to data organization, tracking, and lifecycle management
15.  Evaluation is a core activity in the development of any gen AI system, making it a central part of the iterative development and continuous improvement process. One approach to custom evaluation is using another foundational model as a judge, such as prompting a large language model to score generated text quality across various dimensions, as seen in techniques like AutoSxS. Vertex AI provides evaluation tools ranging from ground truth-based metrics (Automatic Metrics) to LLM-based evaluation (pairwise and pointwise model-based evaluation), and a set of pre-built metrics integrated into the Vertex AI Python SDK.
16. Operationalizing a complete gen AI application involves applying standard software engineering practices like version control and CI/CD to all its components, including new artifacts like prompt templates and chain definitions, while navigating challenges related to testing complexity and reproducibility. 
17. Challenges for CI in gen AI include the difficulty of generating comprehensive test cases for complex, open-ended outputs and issues with reproducibility due to inherent model randomness.
18. 
